...
def item_map(request, **kwargs):
    """Displays edc_map for a subject on the dashboard

    Show the location visually on the edc_map of a subject from the dash by clicking the view edc_map button on the dashboard
    """
    ...
    if not site_mappers.get_registry(mapper_name):
        ...
        map = 1
        if request.GET.get('edc_map'):
            map = int(request.GET.get('edc_map'))
        if not longitude:
