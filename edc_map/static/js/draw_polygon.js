...
var draw_mode = function() {
  ...
  $('#save_map_selection').click(function() {
      ...
      {% if locations %}
	        //check in the locations to see which points are within a polygon then push their identifiers into an array
	        {% for lon, lat, label, icon in locations %}
	        ...
	        {% endfor %}
			window.location.replace("{% url 'sectioning_view_url' %}?identifiers=" + arr + "&section=" + document.getElementById("section").value);
  	{% endif %}
  }
	$('#revert_map').removeAttr('disabled');
