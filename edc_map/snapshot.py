...
class Snapshot(GeoMixin):
    ...
    def __init__(self, filename_prefix, point, map_area,
        ...
        Keyword arguments:
            ...
            map_area: name of area and also the reference key
                to the mapper for the area, e.g community
        ...
        if not os.path.exists(self.image_folder):
            raise FolderDoesNotExist(
                'Map Image folder for edc_map does not exist. '
                'Got {}'.format(self.image_folder))
        ...
        for label, landmark in self.landmarks_by_label.items():
            markers.append(
                self.format_as_markers(
                    landmark.point, label=label, color=color))
        return markers

    def prepare_landmarks(self, mapper_landmarks):
        """Return mapper landmarks as a list of namedtuples
        (adds distance from target).
        """
        landmarks = []
        mapper_landmarks = mapper_landmarks or []
        for landmark in mapper_landmarks:
            landmark = Landmark(
                landmark[LANDMARK_NAME],
                Point(landmark[LATITUDE], landmark[LONGITUDE]), 0)
    ...
    def format_as_markers(self, point, color, label=None):
        ...
        string = template.format(
            color=color, label=label, latitude=point.latitude,
